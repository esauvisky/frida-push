...
def push_and_execute(fname, transport_id=None, push_to_bin=False):
    ...
    cleanup_cmd = [ADB_PATH, "-t", transport_id, "shell", "su", "-c", "rm", "-r", "/data/local/tmp/frida*", "/system/bin/frida-server", "/data/local/tmp/re.frida.server"]
    ...
    subprocess.Popen(cleanup_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).wait()

    subprocess.Popen(mkdir_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).wait()
